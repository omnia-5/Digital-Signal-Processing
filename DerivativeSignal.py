...
def DerivativeSignal():
    ...
    for i in range(len(InputSignal)-1):
        if i == 0:
            second_derivative = InputSignal[i + 1] - 2 * InputSignal[i]
        else:
            second_derivative = InputSignal[i + 1] - 2 * InputSignal[i] + InputSignal[i - 1]

        SecondDrev.append(second_derivative)
